...
class PTIRSFormatter {
    static format(xml, capitalGains, dividends) {
    ...
    static _formatBrowserVersion(xml, capitalGains, dividends) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (anexoJ === null) {
                throw new Error("O Anexo J não foi encontrado na declaração IRS, por favor adicione antes de utilizar esta ferramenta.");
            }
            // No caso das mais valias de capital, verificar o quadro 9.2A
            const quadro9 = anexoJ.querySelector("Quadro09");
            if (quadro9 === null) {
                throw new Error("O Quadro 9 nao foi encontrado na declaração IRS, por favor adicione antes de utilizar esta ferramenta.");
            }
            ...
            capitalGains.forEach((gain) => {
                ...
                const ImpostoPagoNoEstrangeiro = xmlDoc.createElementNS(quadro9.namespaceURI, "ImpostoPagoNoEstrangeiro");
            ...
            if (quadro8 === null) {
                throw new Error("O Quadro 8 nao foi encontrado na declaração IRS, por favor adicione antes de utilizar esta ferramenta.");
            }
            ...
            dividends.forEach((div) => {
                ...
                const ImpostoPagoEstrangeiroPaisFonte = xmlDoc.createElementNS(quadro8.namespaceURI, "ImpostoPagoEstrangeiroPaisFonte");
    ...
    static _formatServerVersion(xml, capitalGains, dividends) {
        return __awaiter(this, void 0, void 0, function* () {
            // Node.js (dynamic import to avoid bundling errors in browser)
            const { DOMParser, XMLSerializer } = yield import("@xmldom/xmldom");
            const parser = new DOMParser();
            const xmlDoc = parser.parseFromString(xml, "text/xml");
            // Verificar se existe o anexo J na declaração
            ...
            if (!quadro9_2A) {
                throw new Error("O Quadro 9.2A nao foi encontrado na declaração IRS, por favor adicione antes de utilizar esta ferramenta.");
            }
            let quadro9_2A_lines = quadro9_2A.childNodes;
            let currNLinha = 951;
            let currNumero = 1;
            ...
            capitalGains.forEach((gain) => {
                ...
                addNode("ImpostoPagoNoEstrangeiro", gain["Imposto retido no estrangeiro"].toString());
                addNode("CodPaisContraparte", gain["País da Contraparte"].split("-")[0].trim());
                quadro9_2A.appendChild(linha);
                currNLinha++;
                currNumero++;
            });
            ...
            dividends.forEach((div) => {
                ...
                addNode("ImpostoPagoEstrangeiroPaisFonte", div["Imposto Pago no Estrangeiro - No país da fonte"].toString());
                quadro8A.appendChild(linha);
                currNLinha++;
                currNumero++;
            });
