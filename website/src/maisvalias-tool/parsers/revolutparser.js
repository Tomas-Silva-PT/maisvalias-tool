...
class RevolutParser {
    loadIsins(fileData) {
        ...
        for (const row of sellRows) {
            ...
            if (!record["Date acquired"])
                continue;
            const ticker = record["Symbol"];
            const isin = record["ISIN"];
            (_a = this.isins) === null || _a === void 0 ? void 0 : _a.push({ ticker, isin });
        }
        for (const row of otherIncomeRows) {
            const record = Object.fromEntries(otherIncomeHeaders.map((h, i) => [h, row[i]]));
            if (!record["Date"])
                continue;
            const ticker = record["Symbol"];
            const isin = record["ISIN"];
            (_b = this.isins) === null || _b === void 0 ? void 0 : _b.push({ ticker, isin });
        }
    ...
    parse(fileData) {
        ...
        rows.forEach((row) => {
            ...
            if (isNaN(exchangeRate) || exchangeRate < Number.EPSILON)
                exchangeRate = 1;
            ...
            const isin = (_b = (_a = this.isins) === null || _a === void 0 ? void 0 : _a.find((i) => i.ticker === ticker)) === null || _b === void 0 ? void 0 : _b.isin;
            if (isin) {
                const transaction = new Transaction(date, time, type, ticker, isin, shares, assetCurrency, amount, amountCurrency, new Revolut(), undefined, fees, exchangeRate);
                // console.log("Transaction: " + JSON.stringify(transaction));
                if (transaction.type)
                    transactions.push(transaction);
            }
            if (!isin && type !== "Buy") {
                throw new Error("Invalid file data: no isin found for " +
                    type +
                    " of ticker " +
                    ticker);
            }
