...
import { YahooFinance } from '../../models/apis/yahoofinance';
...
describe('YahooFinance', () => {
    it('should be able to identify an asset as an ETF', async () => {
        ...
        const assetType = await new YahooFinance().getAssetType(isin);
    ...
    it('should be able to identify multiple assets at the same time', async () => {
        ...
        const assetTypes = await new YahooFinance().getAssetTypeBatch(isins);
    ...
    it('should be able to fetch exchange rate', async () => {
        const exchangeRate = await new YahooFinance().getExchangeRates('USD', 'EUR', [DateTime.fromISO('2023-10-01')]);
        expect(exchangeRate).toBeDefined();
        console.log("Exchange Rate timezone: ", new Date().getTimezoneOffset());
        expect(Math.round(exchangeRate[0].value * 10000) / 10000).toBe(0.9467);
    });
    ...
    it('should be able to fetch multiple exchange rates at the same time', async () => {
        ...
        const exchangeRate = await new YahooFinance().getExchangeRates('USD', 'EUR', dateTimes); //await new YahooFinance().getExchangeRate('USD', 'EUR', new Date('2023-10-01').toISOString().split('T')[0]);
        console.log(exchangeRate);
        console.log("Exchange Rate timezone: ", new Date().getTimezoneOffset());
        expect(exchangeRate).toBeDefined();
        expect(exchangeRate.length).toBe(4);
        expect(exchangeRate[0].date).toBe('2022-04-06');
        expect(Math.round(exchangeRate[0].value * 10000) / 10000).toBe(0.9169);
        expect(exchangeRate[1].date).toBe('2023-03-01');
        expect(Math.round(exchangeRate[1].value * 10000) / 10000).toBe(0.9454);
        expect(exchangeRate[2].date).toBe('2024-08-08');
        expect(Math.round(exchangeRate[2].value * 10000) / 10000).toBe(0.9151);
