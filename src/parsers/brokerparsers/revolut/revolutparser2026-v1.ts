...
class RevolutParser2026_v1 implements BrokerParser, IRevolutParser {
  ...
  canParse(sections: BrokerSection[]): boolean {
    console.log("Checking if file can be parsed with RevolutParser2026_v1...");
    ...
    // USD 12.08
    const hAmount = sample["Total Amount"].split(" ").length === 2;
    const hAmountHasDotDecimal = /\.\d+$/.test(sample["Total Amount"]);
    console.log("Has valid amount format: " + hAmount);
    console.log("Has comma as decimal separator: " + hAmountHasDotDecimal);
    if (!hAmount || !hAmountHasDotDecimal) return false;
...
class RevolutCapitalGainAndDividendParser {
  parse(record: BrokerRecord): Transaction | undefined {
    ...
    const priceShare = parseFloat(record["Price per share"].split(" ")[1]);
    const totalAmount = parseFloat(record["Total Amount"].split(" ")[1]);
    const assetCurrency = record["Currency"];
    const amountCurrency = record["Currency"];
    let exchangeRate = 1 / parseFloat(record["FX Rate"].replace("\r", "")); // Porque a taxa de câmbio vem do EUR para a moeda do ativo, e nós queremos ao contrário
